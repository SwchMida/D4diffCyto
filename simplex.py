...
class Simplex_CLASS:
    ...
    def rand_3d_fixed_T_octaves(self, shape, T, octaves=1, persistence=0.5, frequency=32):
        """
        Returns layered fractal noise in 3D with fixed T.

        If T is scalar -> returns (1, H, W)
        If T is vector of length B -> returns (B, H, W)
        """
        assert len(shape) == 2

        T = np.asarray(T, dtype=np.double)
        T = np.atleast_1d(T).reshape(-1)  # (B,)

        noise = np.zeros((T.size, *shape), dtype=np.double)
        y, x = [np.arange(0, end, dtype=np.double) for end in shape]

        amplitude = 1.0
        for _ in range(octaves):
            # IMPORTANT: pass Z as 1D array (B,), not (B,1,1)
