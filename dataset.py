...
class SimpleImageFolder(Dataset):
    """Simple image-folder dataset for custom datasets.
    ...
    Returns:
        {'image': Tensor[C,H,W], 'label': int, 'filenames': str}

    Images are resized to args['img_size'] and normalized to [-1, 1].
    """

    IMG_EXTS = {'.jpg', '.jpeg', '.png', '.bmp', '.tif', '.tiff', '.webp'}

    def __init__(self, root_dir, img_size=(256, 256), channels=3, label=0, augment=False):
        ...
        for fn in os.listdir(root_dir):
            ...
            if os.path.isfile(p) and os.path.splitext(fn.lower())[1] in self.IMG_EXTS:
                files.append(fn)
        ...
        t += [
            transforms.Resize(self.img_size, transforms.InterpolationMode.BILINEAR),
            transforms.ToTensor(),
        ]
        if self.channels == 3:
    ...
    def __getitem__(self, idx):
        ...
        if self.channels == 3:
            ...
            # If image is RGBA/BGRA, drop alpha
        ...
        return {'image': img_t, 'label': self.label, 'filenames': fn}
...
def init_datasets(ROOT_DIR, args):
    """Return (training_dataset, testing_dataset) based on args['dataset'].

    - Default repo behaviour: NFBS MRI dataset (MRIDataset)
    - Custom patch dataset: dataset='cells' (or 'custom') and data_root='./data'
    """
    dset = str(args.get('dataset', '')).lower()

    # ---- Your folder dataset: data/train/normal, data/test/normal ----
    if dset in {'cells', 'cell', 'custom', 'pkgbm'}:
        data_root = args.get('data_root', f'{ROOT_DIR}data')
        img_size = tuple(args['img_size'])

        ch = args.get('channels', 3)
        if ch == "" or ch is None:
            ch = 3

        training_dataset = SimpleImageFolder(
            root_dir=os.path.join(data_root, 'train', 'normal'),
            img_size=img_size,
            channels=ch,
            augment=True,
            label=0,
        )
        testing_dataset = SimpleImageFolder(
            root_dir=os.path.join(data_root, 'test', 'normal'),
            img_size=img_size,
            channels=ch,
            augment=False,
            label=0,
